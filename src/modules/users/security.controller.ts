import { BadRequestException, Controller, Delete, Get, Param, ParseIntPipe, Post, Res } from '@nestjs/common';
...
export class SecurityController {
  ...
  async logoutFromAllDevices(@CurrentUser() actor: AuthUser, @Res() res: Response) {
    await this.securityService.logoutFromAllDevices(actor.id);
    return ApiResponse.success(res, null, 'Berhasil logout dari semua perangkat.');
  }

  @Get('sessions')
  async viewActiveSessions(@CurrentUser() actor: AuthUser, @Res() res: Response) {
  ...
  async revokeSession(
    ...
    const result = await this.securityService.revokeSession(actor.id, sessionDbId, actor.sessionId);
